...
### Added
- **`registerAuto(address initialOwner, address initialPayoutAddress)`** — New external function that registers a builder code with an auto-generated code (no user-provided code needed)
  - Code is generated on-chain via `keccak256(initialOwner, initialPayoutAddress, block.number, block.prevrandao, nonce)`
